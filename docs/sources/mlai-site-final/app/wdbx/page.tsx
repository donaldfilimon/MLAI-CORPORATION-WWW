...
import { Prose, SplitSection, DeepDive, Glossary, FAQList } from "@/components/content";
import { wdbxContent } from "@/lib/content";
...
export default function WDBXPage() {
  ...
  return (
    <>
      ...
      <Section eyebrow="How it works" title="A vector database that runs where the data lives" accent="wdbx">
        <Prose>
          {wdbxContent.intro.map((para, i) => (
            <p key={i}>{para}</p>
          ))}
        </Prose>
      </Section>

      <SplitSection kicker="Indexing" title="HNSW — logarithmic search, tunable recall" accent="wdbx">
        {wdbxContent.hnsw.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>
      <SplitSection kicker="Persistence" title="Memory-mapped, write-ahead-logged" accent="wdbx">
        {wdbxContent.persistence.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>
      <SplitSection kicker="Compression" title="Quantization — large datasets that fit on-device" accent="wdbx">
        {wdbxContent.quantization.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>

      <Section eyebrow="Where it fits" title="What WDBX is for" accent="wdbx">
        <DeepDive items={wdbxContent.useCases} accent="wdbx" cols={2} />
      </Section>
      ...
      <Section eyebrow="Vocabulary" title="The terms, plainly" accent="wdbx">
        <Glossary items={wdbxContent.glossary} />
      </Section>

      <Section eyebrow="Questions" title="What people ask about WDBX" accent="wdbx">
        <FAQList items={wdbxContent.faq} accent="wdbx" />
      </Section>
