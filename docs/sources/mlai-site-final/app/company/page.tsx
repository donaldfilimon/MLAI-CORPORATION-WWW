...
import { Prose, DeepDive, FAQList, PullQuote } from "@/components/content";
...
export default function CompanyPage() {
  return (
    <>
      ...
      <Section eyebrow="The story" title="Why MLAI exists" accent="abbey">
        <Prose>
          {companyContent.story.map((para, i) => (<p key={i}>{para}</p>))}
        </Prose>
        <div className="mt-8">
          <PullQuote accent="abbey">
            One model was built to answer everything. It buckled. So three were built &mdash; and a substrate to hold them.
          </PullQuote>
        </div>
      </Section>

      <Section eyebrow="How we operate" title="The approach" accent="abbey">
        <DeepDive items={companyContent.approach} accent="abbey" cols={3} />
      </Section>

      <Section eyebrow="Questions" title="About the company" accent="abbey">
        <FAQList items={companyContent.faq} accent="abbey" />
      </Section>
