...
import { Prose, SplitSection, FAQList, PullQuote } from "@/components/content";
import { abbeyContent } from "@/lib/content";
...
export default function AbbeyPage() {
  ...
  return (
    <>
      ...
      <Section eyebrow="How it works" title="An assistant that splits the job" accent="abbey">
        <Prose>
          {abbeyContent.intro.map((para, i) => (
            <p key={i}>{para}</p>
          ))}
        </Prose>
      </Section>
      ...
      <SplitSection kicker="The personas" title="Why warmth and bluntness can't share one set of weights" accent="abbey">
        {abbeyContent.personas.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>
      <SplitSection kicker="Memory" title="Local recall, tamper-evident history" accent="abbey">
        {abbeyContent.memory.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>
      <SplitSection kicker="Efficiency" title="Low latency is a hardware-footprint multiplier" accent="abbey">
        {abbeyContent.efficiency.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
        <PullQuote accent="abbey">
          Fast isn&rsquo;t a feeling. By Little&rsquo;s Law it&rsquo;s how many people you can serve on one machine.
        </PullQuote>
      </SplitSection>
      <SplitSection kicker="Surfaces" title="Where Abbey runs" accent="abbey">
        {abbeyContent.platforms.map((para, i) => (
          <p key={i}>{para}</p>
        ))}
      </SplitSection>

      <Section eyebrow="Questions" title="What people ask about Abbey" accent="abbey">
        <FAQList items={abbeyContent.faq} accent="abbey" />
      </Section>
