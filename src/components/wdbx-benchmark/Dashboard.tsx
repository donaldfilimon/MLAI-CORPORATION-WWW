...
import { CHARTS, AI_CHARTS, ALL_LABELS, AI_MODELS, type ChartDef } from "./charts";
...
function computeScores(charts: ChartDef[], labels: readonly string[]): Record<string, number[]> {
  ...
  for (const chart of charts) {
    ...
    chart.labels.forEach((lbl, i) => {
      const v = data[i];
      const norm = chart.type === "higher" ? (v - min) / range : (max - v) / range;
      if (scores[lbl]) {
    ...
    if (activeTab === "wdbx") {
      const wdbxWins = CHARTS.reduce((count, chart) => {
        const sorted = chart.labels
          .map((l, i) => ({ label: l, value: chart.data[i] }))
          .sort((a, b) => (chart.type === "higher" ? b.value - a.value : a.value - b.value));
...
// ── Chart Card ────────────────────────────────────────────────────────────
const ChartCard = memo(({ chart, visible, isAi }: { chart: ChartDef; visible: Set<string>; isAi: boolean }) => {
  ...
  const rank = useMemo(() => {
    const combined = chart.labels.map((l, i) => ({ label: l, value: chart.data[i] }));
    const sorted = combined.sort((a, b) => chart.type === "higher" ? b.value - a.value : a.value - b.value);
      ...
      <Card variant="glass" className="h-full border-white/10">
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
          ...
          <Badge 
            ...
            className={cn(
              "text-lg px-3 py-1",
              rank === 1 ? "bg-amber-500 text-black border-none" : "text-text-dim border-white/20"
            )}
          >
            #{rank}
          </Badge>
        ...
        <CardFooter className="flex justify-between items-center bg-transparent border-t border-white/5 mt-2">
          <span className="text-xs text-text-dim font-mono uppercase tracking-wider">Metrics validated</span>
          <Button variant="outline" size="sm" onClick={download}>Export PNG</Button>
        </CardFooter>
      </Card>
    </motion.div>
  );
      ...
      <Card variant="glass" className="border-white/5">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="xs" className="flex-1" onClick={showAll}>All</Button>
            <Button variant="secondary" size="xs" className="flex-1" onClick={showOnly}>{heroName}</Button>
          </div>
          <div className="flex flex-wrap gap-2 pt-2">
            {labels.filter(l => l.toLowerCase().includes(searchQuery.toLowerCase())).map(lbl => (
              <Badge
                key={`badge-${lbl}`}
                variant={visible.has(lbl) ? "default" : "outline"}
                className={cn(
                  "cursor-pointer transition-all",
                  visible.has(lbl) ? "bg-primary" : "text-text-dim border-white/10 hover:border-white/30"
                )}
                onClick={() => toggle(lbl)}
              >
                {lbl}
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center text-[10px] font-mono uppercase tracking-widest">
                    <span className={cn(isHero ? "text-primary font-bold" : "text-text-dim")}>{lbl}</span>
                    <span className="text-text-dim/50">Norm. Score</span>
                  </div>
                  <Sparkline scores={scores} color={isHero ? WDBX_COLOR : COMP_COLOR} />
                </div>
              );
            })}
          </div>
...
export function Dashboard() {
  ...
  const [wdbxVisible, setWdbxVisible] = useState<Set<string>>(() => new Set(ALL_LABELS));
  const [aiVisible, setAiVisible] = useState<Set<string>>(() => new Set(AI_MODELS));
  ...
  const toggleLabel = useCallback((lbl: string) => {
    ...
    <div className="shell">
      <main className="main">
        <Sidebar 
          ...
          toggle={toggleLabel} 
          showAll={() => isAi ? setAiVisible(new Set(AI_MODELS)) : setWdbxVisible(new Set(ALL_LABELS))} 
          showOnly={() => isAi ? setAiVisible(new Set(["Abbey System"])) : setWdbxVisible(new Set(["WDBX"]))} 
        ...
        <div className="content-area">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "wdbx" | "ai")} className="w-full">
            <div className="flex items-center justify-between mb-8">
              <TabsList className="bg-surface/50 border border-white/10 p-1">
                <TabsTrigger value="wdbx" className="px-6">WDBX Vector Database</TabsTrigger>
                <TabsTrigger value="ai" className="px-6">Abbey AI Framework</TabsTrigger>
              </TabsList>
              ...
              <Button 
                ...
                onClick={() => {
                  const csv = "Metric,Model,Value\n" + currentCharts.flatMap(c => 
                    c.labels.map((l, i) => `${c.title},${l},${c.data[i]}`)
                  ).join("\n");
          ...
          <footer className="footer">
            © {new Date().getFullYear()} MLAI Corporation · Precision Vector Engineering
          </footer>
        </div>
      </main>
    </div>
