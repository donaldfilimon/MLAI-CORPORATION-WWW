...
import { Network, Database, Brain, Cpu, Zap, ArrowDown } from "lucide-react";

type BlueprintTheme = {
  hex: string;
};

export function BlueprintTab({ theme }: { theme: BlueprintTheme }) {
  const nodes = [
  ...
  return (
    <div className="pt-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="relative h-100 flex flex-col items-center justify-center gap-8">
        {/* WDBX Engine */}
        <motion.div
      ...
      <div className="grid grid-cols-2 gap-4 mt-8">
        <div className="glass-card p-4 space-y-2">
          ...
          <p className="text-[10px] text-text-dim">
            High-throughput vector storage and retrieval via HNSW+DiskANN.
          </p>
        </div>
        <div className="glass-card p-4 space-y-2">
          <h3 className="technical-label text-[10px] flex items-center gap-2">
          ...
          <p className="text-[10px] text-text-dim">
            Dynamic persona-specific context injection at inference edge.
          </p>
        </div>
      </div>
    </div>
