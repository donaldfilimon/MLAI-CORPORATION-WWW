...
export function BlockMath({ tex }: { tex: string }) {
  ...
  if (!katex) {
    return (
      <div className="my-1 overflow-x-auto rounded-lg border border-white/5 bg-white/[0.02] px-5 py-4 font-mono text-xs text-cyan-50/50">
        {tex}
      </div>
    );
  }
  ...
  return (
    <div
      className="my-1 overflow-x-auto rounded-lg border border-white/5 bg-white/[0.02] px-5 py-4 text-[0.95em] text-cyan-50/90"
