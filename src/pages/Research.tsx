...
export const Research = () => {
  ...
  return (
    <section id="research" className="min-h-screen py-24 bg-bg relative overflow-hidden font-sans" aria-labelledby="research-heading">
          ...
          {content.research.tracks.map((track, index) => (
            <Card key={track.name} variant="glass" className="p-6">
              <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl border border-blue-400/20 bg-blue-500/10 text-blue-300">
                {index === 0 ? <GitBranch className="h-5 w-5" /> : index === 1 ? <Layers3 className="h-5 w-5" /> : <GraduationCap className="h-5 w-5" />}
              </div>
              <h3 className="text-lg font-bold text-white mb-2">{track.name}</h3>
              <p className="text-sm leading-relaxed text-text-dim">{track.description}</p>
            </Card>
          ))}
        </CardGrid>
              ...
              <Card variant="glass" className="group flex flex-col justify-between hover:border-blue-500/20 transition-all duration-300 p-0 overflow-hidden">
                <CardContent className="p-8">
                  <div className="flex items-center gap-4 mb-4">
                    <span className={`text-[10px] font-mono font-bold tracking-[0.2em] px-2 py-0.5 rounded-sm uppercase ${tagColors[item.tag] || 'text-blue-400 bg-blue-500/10'}`}>{item.tag}</span>
                    <Separator orientation="vertical" className="h-3 bg-white/20" />
                    <span className="text-[10px] font-mono text-text-dim/60 uppercase tracking-widest">{item.date}</span>
                  </div>
                ...
                <div className="px-8 py-4 border-t border-white/5 flex items-center justify-between bg-white/[0.02]">
                  <span className="text-xs font-mono text-text-dim/40 uppercase tracking-widest">{item.readTime}</span>
                  <Button type="button" variant="link" onClick={openInquiry} className="text-white group-hover:text-blue-400 p-0 h-auto font-bold uppercase tracking-widest text-xs gap-2 transition-all cursor-pointer">
                    Request Paper <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </div>
              </Card>
            </motion.div>
