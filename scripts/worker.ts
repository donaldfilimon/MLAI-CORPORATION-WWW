...
function requireJob(job: Job, workerId: string) {
  ...
  if (!current)
    fail(409, "lease_lost", "Document execution no longer owns this job.");
  if (
    job.kind === "interpret"
      ? !["ready", "partial"].includes(current.status)
      : current.status !== "processing"
  )
    fail(
      409,
      "source_changed",
      "The source is no longer available for processing.",
    );
  const agent = one<{
    status: string;
    selection: string;
    ...
    "SELECT r.status,r.selection,r.workspace_id FROM agent_jobs aj JOIN agent_runs r ON r.id=aj.run_id WHERE aj.job_id=?",
    job.id,
  );
  if (agent) {
    if (
      ["cancelled", "failed"].includes(agent.status) ||
      agent.workspace_id !== job.workspace_id
    )
      fail(409, "agent_stopped", "The requesting agent run has stopped.");
    validateModelSelection(
      job.workspace_id,
      JSON.parse(agent.selection) as ModelSelection,
    );
  }
}
export function acquireDocumentJob(workerId: string): Job | undefined {
  return sqlite
    .transaction(() => {
      run(
        "UPDATE jobs SET status='queued',worker_id=NULL,lease_until=NULL WHERE status='running' AND lease_until<=?",
        Date.now(),
      );
      run(
        "UPDATE jobs SET status='cancelled',worker_id=NULL,lease_until=NULL WHERE status IN ('queued','running') AND id IN (SELECT aj.job_id FROM agent_jobs aj JOIN agent_runs r ON r.id=aj.run_id WHERE r.status IN ('failed','cancelled'))",
      );
      const job = one<Job>(
      ...
      if (!job) return;
      run(
        "UPDATE jobs SET status='running',worker_id=?,lease_until=?,attempts=attempts+1,error=NULL WHERE id=?",
        workerId,
        Date.now() + leaseMs,
        job.id,
      );
...
function parseFailure(stdout: string): string {
  try {
    const parsed = JSON.parse(stdout.trim().split("\n").at(-1) || "{}");
    if (parsed.error) return String(parsed.error);
  } catch {}
  return "Document parsing failed. Check the format and parser installation.";
}
async function parseDocument(job: Job, output: string, signal: AbortSignal) {
  ...
  // extract.py writes exactly one JSON object to stdout; keep only its tail for the error.
  let stdout = "";
  ...
  try {
    ...
    signal.throwIfAborted();
    if (code !== 0) throw new Error(parseFailure(stdout));
...
async function processExtraction(
  ...
  try {
    const output = join(tmp, "result.json"),
      result = await parseDocument(job, output, signal);
    signal.throwIfAborted();
    requireJob(job, workerId);
    ...
    sqlite
      .transaction(() => {
        ...
        run(
          "UPDATE jobs SET status='complete',worker_id=NULL,lease_until=NULL,error=NULL WHERE id=?",
          job.id,
        );
      })
      .immediate();
...
async function processInterpretation(
  ...
  const data = JSON.parse(job.payload) as {
    kind: string;
    compare_with?: string[];
    userId: string;
  ...
  if (
    data.agentRunId ||
    one("SELECT job_id FROM agent_jobs WHERE job_id=?", job.id)
  ) {
  ...
  if (!member || member.role === "viewer")
    fail(
      403,
      "workspace_forbidden",
      "Membership no longer permits interpretation.",
    );
  await runInterpretation(
    ...
    job.document_id,
    { kind: data.kind, compare_with: data.compare_with },
    signal,
    data.selection,
    () => {
      signal.throwIfAborted();
...
export async function processDocumentJob(
  ...
  const controller = new AbortController(),
    signal = AbortSignal.any([stopping, controller.signal]);
  const deadline = setTimeout(
    () =>
      controller.abort(
        new DOMException(
          "Document processing exceeded its time limit.",
          "TimeoutError",
        ),
      ),
    timeoutMs,
  ...
  const heartbeat = setInterval(() => {
    try {
      ...
      requireJob(job, workerId);
      run(
        "UPDATE jobs SET lease_until=? WHERE id=? AND worker_id=? AND attempts=? AND status='running' AND lease_until>?",
        Date.now() + leaseMs,
        job.id,
        workerId,
        job.attempts,
  ...
  try {
    ...
    requireJob(job, workerId);
    if (job.kind === "interpret")
      await processInterpretation(job, workerId, signal);
    ...
    const message = timeout
      ...
      : error instanceof ApiError
        ? error.message
        : job.kind === "interpret"
          ? "Interpretation failed. Check source access and the selected provider."
          : "Document processing failed. Check the format and parser installation.";
    sqlite
      .transaction(() => {
        if (
          !one(
            "SELECT id FROM jobs WHERE id=? AND status='running' AND worker_id=? AND attempts=? AND lease_until>?",
            job.id,
            workerId,
            job.attempts,
            Date.now(),
          ...
          return;
        run(
          "UPDATE jobs SET status=?,error=?,worker_id=NULL,lease_until=NULL WHERE id=?",
          stopping.aborted ? "queued" : "failed",
          stopping.aborted ? null : message,
          job.id,
        );
        if (job.kind !== "interpret")
          run(
            "UPDATE documents SET status=?,progress=?,updated_at=? WHERE id=? AND status='processing'",
            stopping.aborted ? "queued" : "failed",
            stopping.aborted ? "Waiting for worker" : message,
            Date.now(),
            job.document_id,
          );
      })
...
export async function runWorker(stopping: AbortSignal) {
  ...
  const concurrency = Number.isFinite(configuredSlots)
    ? Math.max(1, Math.min(Math.floor(configuredSlots), 4))
    : 2;
  const configuredTimeout =
    Number(process.env.DOCUMENT_TIMEOUT_SECONDS || 300) * 1000;
  const timeoutMs =
  ...
  function start(processing: Promise<void>) {
    const pending = processing
      .catch(() => {
        console.error(
          "Worker task stopped before completion; its lease will recover.",
        );
      })
  ...
  while (!stopping.aborted) {
    ...
    while (!stopping.aborted && active.size < concurrency) {
      ...
      if (agent) {
        preferAgent = false;
        start(processAgentRun(agent, workerId, stopping));
        continue;
      }
      const job = acquireDocumentJob(workerId);
      if (job) {
        preferAgent = true;
        start(processDocumentJob(job, workerId, stopping, timeoutMs));
        continue;
      }
      const next = !preferAgent ? acquireAgentRun(workerId) : undefined;
      if (!next) break;
      preferAgent = false;
      start(processAgentRun(next, workerId, stopping));
    }
    if (!stopping.aborted)
