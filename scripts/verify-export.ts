...
export async function verifyExport(root: string) {
  const manifest = JSON.parse(
    await readFile(join(root, "research-manifest.json"), "utf8"),
  );
  if (
  ...
  async function files(directory: string, prefix = ""): Promise<string[]> {
    ...
    for (const entry of await readdir(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory())
        result.push(
          ...(await files(join(directory, entry.name), relative + "/")),
        );
  ...
  const actual = (await files(root))
    .filter((p) => p !== "research-manifest.json")
    .sort();
  ...
  return { sourceRevision: manifest.sourceRevision, files: actual.length };
}
