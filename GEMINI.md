...
- **Showcase**: `/showcase/*` surfaces use lazy client chunks and cinematic engines under `src/film/`, `src/trailer/`, `src/mega/`, and `src/explainer/` — all four consume `src/film/`'s timeline engine and shared scenes, hosted by `src/components/CinematicShell.tsx`. `/showcase/design` renders the design lab in `src/design/` (real source — not the gitignored `ds-bundle/`); its `mlai-ds-tokens.css` is scoped under a `.mlai-ds` wrapper so it never leaks into `:root`, and `src/index.css` stays the canonical token source.
