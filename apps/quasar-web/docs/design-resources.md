...
| **Social / OG** | `public/og-image.png` — the canonical site-wide card, generated by `scripts/generate-og.py` (`bun run og`); consumed by `app/layout.tsx` (`metadataBase`, theme-color, mask-icon), `manifest.webmanifest`, and pinned by `landing-page.test.ts`. Per-route cards are built at build time by `src/lib/og.tsx` + each dynamic segment's `opengraph-image.tsx`. (The generator draws the PNG from scratch; the unconsumed `public/og-image.svg` was removed on 2026-09-16 and remains in git history.) |
