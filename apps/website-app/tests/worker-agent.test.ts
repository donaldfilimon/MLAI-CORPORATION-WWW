...
run(
  "INSERT INTO user(id,name,email,createdAt,updatedAt) VALUES('actor','Actor','worker@example.test',1,1)",
);
run(
  "INSERT INTO workspaces(id,name,created_at,provider_id) VALUES('workspace','Test',1,'local')",
);
beforeEach(() => {
  vi.restoreAllMocks();
  ...
  run(
    "INSERT OR REPLACE INTO memberships(workspace_id,user_id,role) VALUES('workspace','actor','owner')",
  );
  for (const did of ["source", "comparison"]) {
    run(
      "INSERT INTO documents(id,workspace_id,name,extension,size,status,created_at,updated_at) VALUES(?,'workspace',?,'txt',10,'ready',1,1)",
      did,
      did,
    );
    run(
      "INSERT INTO chunks(id,document_id,workspace_id,ordinal,content,location) VALUES(?,?,'workspace',0,'A fixture fact.','{}')",
      `${did}-chunk`,
      did,
    );
  }
...
function extractionWorker(seconds: number) {
  ...
  writeFileSync(
    join(fixture, "worker/.venv/bin/python"),
    `#!${process.execPath}
...
`,
  ...
  run(
    "INSERT INTO jobs(id,document_id,created_at) VALUES('extract-job','source',1)",
  );
  const child = spawn(
    process.execPath,
    ["--import", import.meta.resolve("tsx"), resolve("scripts/worker.ts")],
    {
      cwd: fixture,
      env: {
        ...process.env,
        ...
        DOCUMENT_TIMEOUT_SECONDS: String(seconds),
      },
  ...
  return {
    ...
    closed: new Promise<void>((resolve) => child.on("close", () => resolve())),
  };
}
it("keeps the extraction deadline active through semantic indexing", async () => {
  ...
  try {
    await until(
      () =>
        one<{ progress: string }>(
          "SELECT progress FROM documents WHERE id='source'",
        )?.progress === "Building local semantic index",
    );
    await until(
      () =>
        one<{ status: string }>(
          "SELECT status FROM jobs WHERE id='extract-job'",
        )?.status === "failed",
      1200,
    );
    expect(
      one("SELECT id FROM chunks WHERE document_id='source'"),
    ).toBeUndefined();
...
it("gracefully stops indexing and truthfully requeues the document", async () => {
  ...
  try {
    ...
    expect(
      one(
        "SELECT status,worker_id,lease_until,error FROM jobs WHERE id='extract-job'",
      ),
    ).toEqual({
      status: "queued",
      worker_id: null,
    ...
    expect(
      one<{ status: string }>("SELECT status FROM documents WHERE id='source'")
        ?.status,
    ).toBe("queued");
    expect(
      one("SELECT id FROM chunks WHERE document_id='source'"),
    ...
    throw new Error(
      `${String(error)}; state=${JSON.stringify(one("SELECT * FROM jobs WHERE id='extract-job'"))}; ${diagnostic()}`,
    );
  } finally {
    child.kill("SIGTERM");
    await closed;
...
function answerAfter(change: () => void) {
  vi.spyOn(globalThis, "fetch").mockImplementation(async () => {
    change();
    return new Response(
      'data: {"choices":[{"delta":{"content":"A fixture fact [1]."}}]}\n\ndata: [DONE]\n\n',
    );
  });
}
...
it("prevents interpretation publication when a comparison source is removed", async () => {
  ...
  await expect(
    runInterpretation(
      ctx,
      "source",
      { kind: "comparison", compare_with: ["comparison"] },
      new AbortController().signal,
    ),
...
it("prevents interpretation publication after the provider configuration changes", async () => {
  answerAfter(() =>
    writeFileSync(
      ...
      JSON.stringify([{ ...provider, model: "changed" }]),
    ),
  );
  await expect(
    runInterpretation(
...
it("does not start embedding work when already cancelled", async () => {
  const controller = new AbortController();
  controller.abort();
  await expect(embed(["fixture"], controller.signal)).rejects.toMatchObject({
    name: "AbortError",
  });
});
it("restores interrupted agents while preserving pending and approved actions", async () => {
  run(
    "INSERT INTO conversations(id,workspace_id,title,created_at,updated_at) VALUES('restore-conversation','workspace','Restore',1,1)",
  );
  run(
  ...
  for (const status of ["pending", "approved"])
    run(
      "INSERT INTO agent_actions(id,run_id,tool,input,target_id,affected,status,created_at) VALUES(?,'restore-run','create_project','{}',?,'[]',?,1)",
      status,
      status,
      status,
    );
  const backup = join(dir, "agent-backup"),
    restored = join(dir, "agent-restored");
  mkdirSync(backup);
  await sqlite.backup(join(backup, "mlai.sqlite"));
  writeFileSync(join(backup, "auth-secret"), "isolated-restored-secret");
  const files = Object.fromEntries(
    ["mlai.sqlite", "auth-secret"].map((name) => [
      name,
      createHash("sha256")
  ...
  try {
    expect(
      copy
        .prepare(
          "SELECT status,worker_id,lease_until,lease_token,active_since,active_ms FROM agent_runs WHERE id='restore-run'",
        )
        .get(),
    ).toEqual({
      status: "queued",
    ...
    expect(
      copy.prepare("SELECT id,status FROM agent_actions ORDER BY id").all(),
    ).toEqual([
