...
This is the canonical guide for `apps/mlai`; `CLAUDE.md` adds Claude-specific
detail and defers to this file on any conflict.

This application lives at `apps/mlai/` in the MLAI integration repository. Run
...
- `next.config.ts` sets `output: "standalone"` and points `outputFileTracingRoot` and `turbopack.root` at the repository root. The Dockerfile builds from the repository root and runs `.next/standalone/apps/mlai/server.js`.
...
- `bun run test` runs Vitest in a Node-only environment, then `bun test tests` for the store, workspace, sidecar, and research checks. There is no jsdom/component-test setup. Focus Vitest with `bunx vitest run src/__tests__/file.test.ts` or `bunx vitest run -t "name"`. A bare `bun test` does not run the Vitest suite.
...
- **`/app/*` runs on a second, separate identity system.** `middleware.ts` gates that route tree on a `better-auth.session_token` cookie, resolved by `apps/mlai/lib/workspace.ts` through `@mlai/store`'s `betterAuth` adapter (`packages/store/src/auth.ts`) over the same Postgres database. This does not replace WorkOS above — every other authenticated route (`/login`, `/console`, `/console/workspace`, `/profile`, admin reads) still runs on it. The merge design (`docs/superpowers/specs/2026-09-22-single-app-merge-design.md`) decided to retire WorkOS in favor of Better Auth everywhere, but that retirement is not done; treat both as live until it is. `createAuth`'s secret is a hardcoded dev constant today, unlike `SESSION_SECRET`, which fails closed in production.
