...
| **Social / OG** | `public/og-image.png` — the canonical site-wide card, generated by `scripts/generate-og.py` (`bun run og`); consumed by `app/layout.tsx` (`metadataBase`, theme-color, mask-icon), `manifest.webmanifest`, and pinned by `landing-page.test.ts`. Per-route cards are built at build time by `src/lib/og.tsx` + each dynamic segment's `opengraph-image.tsx`. (`public/og-image.svg` exists but is consumed by nothing — the generator draws the PNG from scratch and never reads it; treat the SVG as an unconsumed design source, not the canonical asset.) |
| **Site content (single source of truth)** | `src/data/categories/*` (about, products, research, blog, team, platform, services, stats, faq, industries, changelog), aggregated by `src/data/index.ts` |
