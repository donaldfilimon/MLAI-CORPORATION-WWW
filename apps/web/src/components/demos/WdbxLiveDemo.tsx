  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-surface/40 backdrop-blur-xl shadow-[0_4px_16px_0_rgba(0,0,0,0.2),inset_0_1px_0_0_rgba(255,255,255,0.06)]">
      ...
      <div className="flex items-center gap-2 border-b border-white/5 px-4 py-3">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-cyan-400/70" aria-hidden="true" />
      ...
      <div className="p-5 md:p-6">
        ...
        <div className="flex gap-2">
          <input
            className="min-w-0 flex-1 rounded-lg border border-white/10 bg-bg/60 px-4 py-2.5 font-mono text-sm text-white placeholder:text-text-dim/50 outline-none focus:border-cyan-400/40 focus:ring-2 focus:ring-cyan-500/20"
            value={query}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {PRESETS.map((p) => (
            <button
              ...
              className="rounded-full border border-white/10 px-3 py-1 font-mono text-[11px] text-text-dim transition-colors hover:border-cyan-500/30 hover:text-cyan-300"
            >
              {p}
            </button>
          ))}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {blocks.map((b, i) => (
              <span key={b.height} className="contents">
                {i > 0 && (
                  <span className="font-mono text-[10px] text-cyan-400/50" aria-hidden="true">
                    ─⛓─
                  </span>
                )}
                <span className="rounded-lg border border-cyan-500/15 bg-cyan-500/5 px-2.5 py-1.5">
