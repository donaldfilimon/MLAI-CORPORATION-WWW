...
- **Do not collapse `FAQ.tsx`/`Stats.tsx` onto `FAQList`/`StatBlock`** — they overlap in subject, not job (animated Accordion + sticky layout; operating-model cards). Both pairs coexist deliberately. Performance figures require a reproducible repository harness and published methodology.
