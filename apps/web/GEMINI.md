...
- **Auth/Admin**: `mlai_session` is an iron-session cookie. Admin reads (`GET /api/inquiries`, `GET /api/telemetry/summary`) require session + `ADMIN_EMAILS`; MFA is enabled by `ADMIN_REQUIRE_MFA=true`. A set `LLM_PROVIDER` is authoritative: without that provider's key, `/api/llm/*` serves the scaffold even if the other key is present — leave it blank for auto-detection.
...
- **`APP_URL` is REQUIRED in every deployed environment** — `REDIRECT_URI` derives from it and `--set-env-vars` replaces the whole env set, so unset ships a localhost redirect URI and breaks login for everyone. Deploy now fails fast. SQLite still has no mounted volume; `--max-instances=1` is a stopgap.
