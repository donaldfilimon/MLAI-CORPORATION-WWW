// forked from design-sync lib/dts.mjs - resolve types from cfg.entry + real .tsx
// sources, because this repo is a Next.js app with no built .d.ts tree.
//
// Upstream computes the ts-morph entry as `pkgDir/<pkg.types || index.d.ts>` and
// only globs `**/*.d.ts`. mlai-corporation-www ships neither, so every component
// emitted `[key: string]: unknown` as its entire API contract - which is exactly
// what the Claude Design agent codes against. Proven on donald-filimon-sites
// 2026-09-06: the same fork turned that repo's stubs into real prop unions.
//
// This fork changes three things and nothing else:
//   1. entry falls back to cfg.entry (src/components/ds.ts).
//   2. the project also loads srcDir's .tsx/.ts.
//   3. baseUrl+paths come from cfg.tsconfig so `@/...` resolves.
//
// SAFETY NOTE for this repo specifically: tsconfig maps `react-router-dom` to
// src/lib/router-compat.tsx, which imports next/link and reads process.env
// __NEXT_* at module scope. That trap is a BUNDLING failure, not a typing one -
// this fork only feeds ts-morph, never esbuild, so it cannot re-arm it. Verify
// after any rebuild that the bundle header still reports 41 components on
// window.MlaiLab; [BUNDLE_EXPORT] N/N is the symptom if it ever does.
import { readFileSync as __rfs, existsSync as __ex } from 'node:fs';
import { dirname as __dn, join as __jn, resolve as __rs } from 'node:path';
const __cfgPath = new URL('../config.json', import.meta.url).pathname;
const __cfg = JSON.parse(__rfs(__cfgPath, 'utf8'));
const __cfgDir = __dn(__dn(__cfgPath));            // repo root (.design-sync/..)
const __entry = __cfg.entry ? __rs(__cfgDir, __cfg.entry) : null;
const __srcDir = __cfg.srcDir ? __rs(__cfgDir, __cfg.srcDir) : null;
let __paths = {}, __baseUrl = __cfgDir;
if (__cfg.tsconfig) {
  try {
    const tj = JSON.parse(__rfs(__rs(__cfgDir, __cfg.tsconfig), 'utf8').replace(/^\s*\/\/.*$/gm, ''));
    const tsDir = __dn(__rs(__cfgDir, __cfg.tsconfig));
    __paths = tj.compilerOptions?.paths ?? {};
    if (tj.compilerOptions?.baseUrl) __baseUrl = __rs(tsDir, tj.compilerOptions.baseUrl);
  } catch { /* fall back to repo-root resolution */ }
}
...
function projectFor(pkgDir, typesRoot) {
  ...
  // [OVERRIDE] no shipped .d.ts entry -> use the configured source barrel.
  if (!existsSync(entry) && __entry && __ex(__entry)) entry = __entry;
  ...
  // [OVERRIDE] an app has no .d.ts tree - load the components themselves.
  if (__srcDir && __ex(__srcDir)) {
    project.addSourceFilesAtPaths([`${__srcDir}/**/*.tsx`, `${__srcDir}/**/*.ts`, `!${__srcDir}/**/*.test.*`, `!**/node_modules/**`]);
    console.error(`  [OVERRIDE] +${project.getSourceFiles().length} file(s) after loading ${__srcDir}`);
  }
