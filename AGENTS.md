...
- Generation is fixed to stable `gemini-3.7-flash` through the authenticated Cloudflare AI Gateway `/compat` endpoint. The app has no direct Gemini/Anthropic production bypass. Keep payload collection and gateway caching disabled on every request, and never serialize the user's email into provider messages.
- Production admin reads fail closed without `ADMIN_EMAILS`; `ADMIN_REQUIRE_MFA=true` additionally requires a WorkOS factor, fresh authentication, active organization membership, and `WORKOS_MFA_POLICY_VERIFIED=true` after manual production-policy verification.
