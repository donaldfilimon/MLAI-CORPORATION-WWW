...
- `bun run crawl` / `bun run smoke` - run link crawl and API smoke scripts against a running dev server. `bun run smoke` keeps two expensive cases opt-in: `SMOKE_WRITE=1` for the inquiry happy-path POST (writes to `inquiries.db`) and `SMOKE_RATELIMIT=1` for the 61-request burst asserting the csp-report 429.
...
The suite runs in the `node` environment with no jsdom or DOM-testing setup, so it covers pure server and library logic only — one file per module, plus the policy-pinning `content.test.ts` (Zod schema, unique URL-safe slugs), `workos-admin.test.ts` (fail-closed admin policy, MFA gating, open-redirect guard), and `session.test.ts` (leftmost-XFF client IP, public-user token stripping, the fail-closed `SESSION_SECRET` policy, cookie flags, and rejection of a sealed cookie replayed from a different IP or User-Agent). Do not pin a file or test count in these docs — it drifts every commit. Adding a component test means adding that environment first. Use `bunx vitest run <file>` or `bunx vitest run -t "<name>"` for a single test; do not use `bun test`, which invokes Bun's own runner instead of Vitest.
