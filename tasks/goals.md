...
- **2026-09-22:** Phases 5–7 are on this commit. `apps/mlai` is the only Next app: it serves the public Quesar site and `/app/*`. Better Auth sessions are created and read back through Postgres in `@mlai/store`; an unsigned `/app/*` request is refused; episode, receipt, and vector reads fail closed when the WDBX gateway is absent. `@quasar/service` and `sidecars/python-worker` are URL sidecars (no child-process spawn). Quasar screens are routes. The research collection is a hashed build output. Production responses send the extended Content-Security-Policy and do not add `'unsafe-eval'`. The Capacitor shell loads this app's web build and includes Apple sign-in, secure storage, and the CloudKit plugin (`cap sync` exit 0). `apps/website-app`, `apps/mobile`, `apps/quasar/apps/quasar`, and `apps/research-sites` are removed from the tree, workspaces, `check:*`, CI, and topology. `bun install --frozen-lockfile --lockfile-only` and `bun run check` exited 0. Signed-device CloudKit sync, live Apple identity, and store submission stay external. Hosted Cloud Run cutover, the self-hosted runner, and zoom/cinematic/signed-in acceptance stay blocked.
