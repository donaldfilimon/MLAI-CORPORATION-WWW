...
- **2026-09-22:** Phase 3 is landed. `apps/quasar-web` depends on Next 16, TypeScript 7, Vitest 5, and a React 19 caret (`13ae3c2`); `bun install --frozen-lockfile --lockfile-only` and `bun run check:web` passed on `main`. The token-source slice is landed in `3ff0518`: the shared sheets are generated from `packages/design-tokens`, and `bun run tokens:generate` does not drift them. `@mlai/ui` dialog, sheet, navigation menu, separator, and the button/badge slot use `@base-ui/react`; the package no longer declares `radix-ui`. `cmdk` still backs the command palette and brings its own `@radix-ui/react-*` packages. Next: port website-app screens onto the shared UI or into quasar-web (the rest of Phase 4).
