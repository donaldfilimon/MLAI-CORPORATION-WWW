...
export default function RootLayout({ children }: { children: ReactNode }) {
  return (
    <html lang="en" className="scroll-smooth dark" suppressHydrationWarning>
      <head>
        {/* Display (Outfit) + mono (JetBrains Mono) load async via the
            print→all swap so they never block first paint; body font (Geist)
            is self-hosted via @fontsource (imported above). */}
        ...
        <link
          ...
          media="print"
          // @ts-expect-error — onload as a plain attribute string for the swap trick
